...
def run_training_process(
    *,
    ...
    # [KAGGLE FORGE] Keep Kaggle behavior strictly opt-in via env vars.
    # Studio can pass explicit flags per run; we map them to environment here.
    if config.get("kaggle_timebomb_hours") is not None:
        os.environ["UNSLOTH_KAGGLE_TIMEBOMB"] = str(config.get("kaggle_timebomb_hours"))
    if config.get("kaggle_auto_vram", False):
        os.environ["UNSLOTH_AUTO_VRAM"] = "1"
    if config.get("kaggle_ghost_cache", False):

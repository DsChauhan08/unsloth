...
from unsloth.kaggle_runtime import (
    UnslothKaggleTimebombCallback,
    clear_vram,
    derive_vram_fallback_batch_params,
    ghost_cache_paths,
    is_cuda_oom_error,
    stable_dataset_fingerprint,
)
from unsloth.models.kaggle_forge import (
    kaggle_auto_vram_enabled,
    kaggle_ghost_cache_dir,
    kaggle_ghost_cache_enabled,
...
class UnslothTrainer:
    ...
    def _resolve_kaggle_timebomb_hours(
        self, training_args: dict[str, Any]
    ) -> Optional[float]:
        explicit = training_args.get("kaggle_timebomb_hours", None)
        if explicit is not None:
            try:
                ...
                pass
        return kaggle_timebomb_hours()

    def _attach_kaggle_timebomb_if_needed(
        self, trainer, training_args: dict[str, Any]
    ) -> None:
        hours = self._resolve_kaggle_timebomb_hours(training_args)
        if hours is None or hours <= 0:
            return
        if any(
            isinstance(cb, UnslothKaggleTimebombCallback) for cb in trainer.callbacks
        ):
            return
        trainer.add_callback(
            UnslothKaggleTimebombCallback(
                hours=hours,
                start_time=self.training_start_time or time.time(),
            )
        )
        logger.info(
            "[KAGGLE FORGE] Timebomb enabled: graceful stop after %.3f hours", hours
        )

    def _auto_vram_enabled(self, training_args: dict[str, Any]) -> bool:
        if bool(training_args.get("kaggle_auto_vram", False)):
            return True
        return kaggle_auto_vram_enabled()

    def _train_with_auto_vram(self, training_args: dict[str, Any]) -> None:
        """Train with optional OOM retries by lowering per-device batch size."""
        requested_batch = int(training_args.get("batch_size", 2) or 2)
        requested_accum = int(training_args.get("gradient_accumulation_steps", 4) or 4)

        if not self._auto_vram_enabled(training_args):
            self.trainer.train()
            return

        candidates = derive_vram_fallback_batch_params(requested_batch, requested_accum)
        for attempt_idx, (batch_size, grad_accum) in enumerate(candidates, start=1):
            self.trainer.args.per_device_train_batch_size = batch_size
            self.trainer.args.gradient_accumulation_steps = grad_accum
            logger.warning(
                "[KAGGLE FORGE] Auto VRAM attempt %d/%d with batch_size=%d, grad_accum=%d",
                attempt_idx,
                len(candidates),
                batch_size,
                grad_accum,
            ...
            self._update_progress(
                status_message=(
                    f"Auto VRAM attempt {attempt_idx}/{len(candidates)} "
            ...
            try:
                ...
                if attempt_idx > 1:
                    logger.warning(
                        "[KAGGLE FORGE] Training recovered after OOM with "
                        "batch_size=%d, grad_accum=%d",
                        batch_size,
                        grad_accum,
                    )
                return
                ...
                if not is_cuda_oom_error(exc):
                    raise
                logger.warning(
                    "[KAGGLE FORGE] OOM on attempt %d/%d: %s",
                    attempt_idx,
                    len(candidates),
                    exc,
                )
    ...
    def _ghost_cache_is_enabled(
        self, training_args: Optional[dict[str, Any]] = None
    ) -> bool:
        if training_args and bool(training_args.get("kaggle_ghost_cache", False)):
            return True
        return kaggle_ghost_cache_enabled()

    def _build_ghost_cache_fingerprint(
        self,
        *,
        dataset_source: str,
        ...
        return stable_dataset_fingerprint(
            ...
            is_audio_vlm=self.is_audio_vlm,
        )

    @staticmethod
    def _cacheable_payload(train_payload, eval_payload) -> bool:
        if isinstance(train_payload, Dataset):
            return eval_payload is None or isinstance(eval_payload, Dataset)
        if isinstance(train_payload, dict) and isinstance(
            train_payload.get("dataset"), Dataset
        ):
            return eval_payload is None or isinstance(eval_payload, Dataset)
        return False

    def _try_load_ghost_cache(self, fingerprint: str):
        cache_root = kaggle_ghost_cache_dir()
        cache_dir, metadata_path = ghost_cache_paths(cache_root, fingerprint)
        ...
        try:
            ...
            if kind == "dataset":
                train_payload = load_from_disk(str(cache_dir / "train_dataset"))
            elif kind == "dict_dataset":
                train_payload = {
                    "dataset": load_from_disk(str(cache_dir / "train_dataset")),
                    "success": metadata.get("success", True),
                    "detected_format": metadata.get("detected_format", "unknown"),
            ...
            eval_payload = None
            if metadata.get("has_eval", False):
                eval_payload = load_from_disk(str(cache_dir / "eval_dataset"))

            logger.info("[KAGGLE FORGE] Ghost cache hit: %s", cache_dir)
            self._update_progress(status_message="Loaded dataset from Ghost Cache")
            return (train_payload, eval_payload)
        except Exception as exc:
            logger.warning("[KAGGLE FORGE] Ghost cache load failed, ignoring: %s", exc)
            return None

    def _save_ghost_cache(self, fingerprint: str, train_payload, eval_payload) -> None:
        if not self._cacheable_payload(train_payload, eval_payload):
            return

        cache_root = kaggle_ghost_cache_dir()
        cache_dir, metadata_path = ghost_cache_paths(cache_root, fingerprint)
        tmp_cache_dir = cache_dir.with_name(cache_dir.name + ".tmp")
        try:
            ...
            metadata: dict[str, Any] = {
                "version": 1,
                "has_eval": bool(eval_payload is not None),
            }
            if isinstance(train_payload, Dataset):
                train_payload.save_to_disk(str(tmp_cache_dir / "train_dataset"))
                metadata["kind"] = "dataset"
            elif isinstance(train_payload, dict) and isinstance(
                train_payload.get("dataset"), Dataset
            ):
                train_payload["dataset"].save_to_disk(
                    str(tmp_cache_dir / "train_dataset")
                )
                metadata.update(
                    {
                        "kind": "dict_dataset",
                        "success": train_payload.get("success", True),
                        "detected_format": train_payload.get("detected_format"),
                        "final_format": train_payload.get("final_format"),
                    }
                )
            else:
                return
            ...
            if eval_payload is not None:
                eval_payload.save_to_disk(str(tmp_cache_dir / "eval_dataset"))

            metadata_path_tmp = tmp_cache_dir / "meta.json"
            metadata_path_tmp.write_text(json.dumps(metadata, indent=2))

            if cache_dir.exists():
                shutil.rmtree(cache_dir, ignore_errors=True)
            tmp_cache_dir.replace(cache_dir)
            logger.info("[KAGGLE FORGE] Ghost cache saved: %s", cache_dir)
        except Exception as exc:
            logger.warning("[KAGGLE FORGE] Ghost cache save failed: %s", exc)
            shutil.rmtree(tmp_cache_dir, ignore_errors=True)
    ...
    def load_and_format_dataset(
        ...
        try:
            ...
            ghost_enabled = self._ghost_cache_is_enabled(training_args)
            ghost_fingerprint = None
            if ghost_enabled:
                ghost_fingerprint = self._build_ghost_cache_fingerprint(
                ...
                cached = self._try_load_ghost_cache(ghost_fingerprint)
            ...
            if self._audio_type == "csm":
                ...
                if ghost_enabled and ghost_fingerprint is not None:
                    self._save_ghost_cache(ghost_fingerprint, processed, None)
                ...
                payload = {"dataset": processed, "final_format": "audio_bicodec"}
                if ghost_enabled and ghost_fingerprint is not None:
                    self._save_ghost_cache(ghost_fingerprint, payload, None)
                return (payload, None)

            elif self._audio_type == "dac":
                processed = self._preprocess_dac_dataset(dataset, custom_format_mapping)
                payload = {"dataset": processed, "final_format": "audio_dac"}
                if ghost_enabled and ghost_fingerprint is not None:
                    self._save_ghost_cache(ghost_fingerprint, payload, None)
                return (payload, None)
                ...
                if ghost_enabled and ghost_fingerprint is not None:
                    self._save_ghost_cache(ghost_fingerprint, formatted, None)
            ...
            if ghost_enabled and ghost_fingerprint is not None:
                self._save_ghost_cache(ghost_fingerprint, dataset_info, eval_dataset)
    ...
    def _train_worker(self, dataset: Dataset, **training_args):
        ...
        try:
            ...
            if bool(training_args.get("kaggle_auto_vram", False)):
                os.environ["UNSLOTH_AUTO_VRAM"] = "1"
            if bool(training_args.get("kaggle_ghost_cache", False)):
                os.environ["UNSLOTH_GHOST_CACHE"] = "1"
            if training_args.get("kaggle_timebomb_hours") is not None:
                os.environ["UNSLOTH_KAGGLE_TIMEBOMB"] = str(
                    training_args.get("kaggle_timebomb_hours")
                )
            ...
            if self._audio_type == "csm":
                ...
                self._attach_kaggle_timebomb_if_needed(self.trainer, training_args)
                ...
                self._train_with_auto_vram(training_args)

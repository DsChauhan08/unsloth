...
class TrainingStartRequest(BaseModel):
    ...
    kaggle_timebomb_hours: Optional[float] = Field(
        None,
        description="[KAGGLE FORGE] Graceful wallclock stop in hours (env: UNSLOTH_KAGGLE_TIMEBOMB)",
    )
    kaggle_auto_vram: bool = Field(
        False,
        description="[KAGGLE FORGE] Enable auto OOM VRAM fallback (env: UNSLOTH_AUTO_VRAM)",
    )
    kaggle_ghost_cache: bool = Field(
        False,
        description="[KAGGLE FORGE] Enable dataset ghost cache (env: UNSLOTH_GHOST_CACHE)",
    )
